...
const showFlockComments = async (payload) => {
    try {
        ...
        const response = {
            results: commentsResp,
            count: commentsCount[0].total_comments,
        };
        return { response: response, statusCode: 200 };
    } catch (error) {
        return {
            response: `Exception in show all flock comments : ${error}`,
            statusCode: 500,
        };
    }
};
...
const addFlockLike = async (userId, payload) => {
    try {
        ...
        if (likeResp) return { statusCode: 204 };
        else return { response: 'Invalid flockId', statusCode: 400 };
    } catch (error) {
        return {
            response: `Exception in add like on flock : ${error}`,
            statusCode: 500,
        };
    }
};
...
const saveFlock = async (userId, payload) => {
    try {
        ...
        flockId = new ObjectId(flockId);
        userId = new ObjectId(userId);
        const saveResp = await db
            .collection('users')
            .findOneAndUpdate({ _id: userId }, { $push: { savedFlock: flockId } }, { returnNewDocument: 'true' });
        if (saveResp) return { statusCode: 204 };
        else return { response: 'User not exist.', statusCode: 400 };
    } catch (error) {
        return {
            response: `Exception in saving flock : ${error}`,
            statusCode: 500,
        };
    }
};
...
const viewSavedFlock = async (userId) => {
    try {
        ...
        if (savedFlock) return { response: savedFlock[0].myItems, statusCode: 200 };
        else return { response: `User not exist`, statusCode: 400 };
    } catch (error) {
        return {
            response: `Exception in saving flock : ${error}`,
